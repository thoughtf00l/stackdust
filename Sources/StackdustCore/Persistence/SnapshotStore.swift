...
///
/// Files live in `~/Library/Caches/Stackdust` (injectable for tests), named by a hash of the
...
public struct SnapshotStore: Sendable {
    ...
    ///   (`~/Library/Caches/Stackdust`).
    public init(directory: URL? = nil) {
        if let directory {
            self.directory = directory
        } else {
            let caches = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0]
            self.directory = caches.appendingPathComponent("Stackdust", isDirectory: true)
            Self.adoptLegacyDirectory(
                from: caches.appendingPathComponent("DiscFree", isDirectory: true),
                to: self.directory
            )
        }
    }

    /// Moves the cache directory left behind by the app's previous name (DiscFree) to the
    /// current location, once. Cache semantics make a failed move harmless: the old
    /// directory is simply ignored and snapshots regenerate on the next scan.
    private static func adoptLegacyDirectory(from old: URL, to new: URL) {
