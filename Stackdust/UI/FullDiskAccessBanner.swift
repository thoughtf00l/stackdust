...
/// which Stackdust intentionally does not download (they hold almost no local disk space).
struct CloudEvictedIndicator: View {
    ...
    var body: some View {
        Label("\(count) in iCloud", systemImage: "icloud")
            .foregroundStyle(.secondary)
            .help("Directories whose content is evicted to iCloud. Stackdust does not download them: they occupy almost no local disk space. Their sizes are not included.")
    }
}
